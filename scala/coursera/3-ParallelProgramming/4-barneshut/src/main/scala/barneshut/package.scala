...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    val centerX: Float = (quads.map(_.centerX).min + quads.map(_.centerX).max) / 2
    val centerY: Float = (quads.map(_.centerY).min + quads.map(_.centerY).max) / 2
    val size: Float = 2 * se.size
    val mass: Float = quads.foldLeft(0.0f)(_ + _.mass)
    val massX: Float = if (mass == 0) centerX
      else quads.foldLeft(0.0f) { case (sum, quad) => sum + quad.mass * quad.massX } / mass
    val massY: Float = if (mass == 0) centerY
      else quads.foldLeft(0.0f) { case (sum, quad) => sum + quad.mass * quad.massY } / mass
  ...
  extends Quad {
    val mass: Float = bodies.foldLeft(0.0f)(_ + _.mass)
    val massX: Float = bodies.foldLeft(0.0f) { case (sum, b) => sum + b.mass * b.x } / mass
    val massY: Float = bodies.foldLeft(0.0f) { case (sum, b) => sum + b.mass * b.y } / mass
